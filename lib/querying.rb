...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM ((series INNER JOIN books ON books.series_id = series.id) INNER JOIN character_books ON character_books.book_id = books.id) INNER JOIN characters ON character_books.character_id = characters.id GROUP BY characters.species HAVING characters.species = 'human' ORDER BY COUNT(characters.species) DESC LIMIT 1"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(characters.name) FROM (characters INNER JOIN character_books ON character_books.character_id = characters.id) INNER JOIN books ON books.id = character_books.book_id GROUP BY characters.name ORDER BY COUNT(books.title) DESC, characters.name ASC"
end
